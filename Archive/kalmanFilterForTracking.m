...
function showDetections()
  ...
  while hasFrame(utilities.videoReader)
    ...
    pause(1/utilities.videoReader.FrameRate);
  end % while
...
function [detection, isObjectDetected] = detectObject(frame)
  grayImage = rgb2gray(im2single(frame));%turn gbd to gray image
  utilities.foregroundMask = step(utilities.foregroundDetector, grayImage); %Process inputs using the object algorithm
...
function utilities = createUtilities(param)
  ...
  utilities.videoReader = VideoReader('clip_surgery.mp4');
  utilities.videoPlayer = vision.VideoPlayer('Position', [100,100,400,300]);
%  utilities.videoPlayer = vision.VideoPlayer('Position');
  utilities.foregroundDetector = vision.ForegroundDetector(...
    'NumTrainingFrames', 100, 'InitialVariance', param.segmentationThreshold);
